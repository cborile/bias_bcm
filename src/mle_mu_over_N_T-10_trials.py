...
n_ic = 10
n_trials = 10
t_range = np.array([100, 1000, 5000, 10000, 20000]) # 
n_range = np.array([1000]) # 3, 5, 10, 100, 500
mu_star_list = np.zeros((n_ic, len(t_range), len(n_range), n_trials))
...
for c, T in enumerate(t_range):
    for ceps, N in enumerate(n_range):
        ...
        df_tmp.to_csv(f'../outputs/mle_mu_over_N_T_{n_trials}_{eps}_{mu}_{beta}_{N}_{T}_{generation_seed}.csv')
